document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleFormSubmit(e) {
        ...
        const newEntry = {
            ...
            nombre_embarcacion: formData.get('nombre_embarcacion'),
            tipo_embarcacion: formData.get('tipo_embarcacion'),
        ...
        state.embarcacionesData.push(newEntry);
        renderTable();
        hideRegisterForm();
    ...
    function renderTableRows(data) {
        data.forEach(embarcacion => {
            const row = document.createElement('tr');
            row.className = 'hover:bg-white hover:bg-opacity-10';
            row.innerHTML = `
                <td class="px-4 py-3 text-center">${embarcacion.item}</td>
                <td class="px-4 py-3 text-center">${embarcacion.fecha || ''}</td>
                <td class="px-4 py-3">${embarcacion.piloto || ''}</td>
                <td class="px-4 py-3">${embarcacion.nombre_embarcacion || ''}</td>
                <td class="px-4 py-3 text-center">${embarcacion.tipo_embarcacion || ''}</td>
                <td class="px-4 py-3 text-center">${embarcacion.hora_entrada || ''}</td>
                <td class="px-4 py-3 text-center">${embarcacion.hora_salida || ''}</td>
                <td class="px-4 py-3 text-center">${embarcacion.fecha_salida || ''}</td>
                <td class="px-4 py-3">${embarcacion.observaciones || ''}</td>
                <td class="px-4 py-3 text-center">
                    <button class="bg-blue-600 hover:bg-blue-700 text-white px-2 py-1 rounded salida-btn" data-id="${embarcacion.item}">Registrar Zarpe</button>
                    <button class="bg-red-600 hover:bg-red-700 text-white px-2 py-1 rounded delete-btn" data-id="${embarcacion.item}">Eliminar</button>
                </td>
            `;
    ...
    function exportExcel() {
        if (state.embarcacionesData.length === 0) {
            alert('No hay datos para exportar');
            return;
        }
        const headers = Object.keys(state.embarcacionesData[0]);
        const data = state.embarcacionesData.map(embarcacion => headers.map(header => embarcacion[header]));
        const workbook = XLSX.utils.book_new();
        const worksheet = XLSX.utils.aoa_to_sheet([headers, ...data]);
        XLSX.utils.book_append_sheet(workbook, worksheet, "Registro");
        XLSX.writeFile(workbook, "registro_embarcaciones.xlsx");
        alert('Datos exportados a Excel correctamente');
    }

    function handleTableClick(e) {
        if (e.target.classList.contains('salida-btn')) {
            state.embarcacionSalidaId = parseInt(e.target.dataset.id);
            showSalidaForm();
    ...
    function handleSalidaSubmit(e) {
        ...
        const embarcacion = state.embarcacionesData.find(v => v.item === state.embarcacionSalidaId);
        if (embarcacion) {
            embarcacion.hora_salida = formData.get('hora_salida');
            embarcacion.fecha_salida = formData.get('fecha_salida');
            renderTable();
            hideSalidaForm();
            alert('Zarpe registrado exitosamente');
        }
    }
    ...
    function handleDelete(e) {
        if (e.target.classList.contains('delete-btn')) {
            ...
            if (confirm('¿Está seguro de que desea eliminar este registro?')) {
                state.embarcacionesData = state.embarcacionesData.filter(embarcacion => embarcacion.item !== id);
                renderTable();
            }
        }
    }
